...
def segment_colonies(image, min_size=None, max_size=None, lw_ratio=None,
 img_occupancy=None, min_variance=None):
 ...
 objs = [x for x in objs if len(get_dat_above_thr(x,image))/get_size_obj_2d(x) > img_occupancy]
 objs = [x for x in objs if sc.ndimage.variance(image[x])/np.mean(image[x]) > min_variance]
 obj_locs = [(x[1].start + ((x[1].stop - x[1].start)/2) ,
 x[0].start - ((x[0].start - x[0].stop)/2)) for x in objs]
...
rein_dat, rein_locs, rein_objs, rein_lab_image = \
segment_colonies(rein_image, max_size = 1200, min_size = 10)

#segmetn colonies for smithii image
smithii_dat, smithii_locs, smithii_objs, smithii_lab_image = \
segment_colonies(smithii_image, max_size = 1200, min_size = 20)
...
for n in range(len(paths)):
 ...
 plt.plot(times,mean_species_dat,marker='o',linestyle='',
 ms=4, solid_capstyle='round',label=data_labels[n])
